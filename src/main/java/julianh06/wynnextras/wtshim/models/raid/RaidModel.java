...
/*
 *
 * Ported for the WynnExtras standalone compat shim (wtshim). This is a faithful port of the
 * Wynntils RaidModel — event-driven detection, room timestamps and event posting are byte-for-byte
 * behaviourally identical. The only changes are Yarn/shim adaptations, each marked "compat shim":
 *   - super(List.of()) dropped (shim Model has a no-arg constructor);
 *   - Mojmap Component/ChatFormatting/ItemStack -> Yarn Text/Formatting/ItemStack;
 *   - LoreUtils.getLore(...) returns List<Text> in the shim, wrapped via StyledText.fromComponent;
 *   - ItemStack#getHoverName -> #getName;
 *   - interruptRaid message sent via McUtils.sendMessageToClient (shim has no sendWynntilsPrefixMessage);
 *   - @Persisted Storage fields are in-memory only (see core/persisted/storage/Storage).
 * DamageDealtEvent is never posted (see that class); room damage therefore stays 0.
 */
package julianh06.wynnextras.wtshim.models.raid;
...
import julianh06.wynnextras.wtshim.core.mod.TickSchedulerManager.ScheduledTask;
import julianh06.wynnextras.wtshim.core.persisted.Persisted;
import julianh06.wynnextras.wtshim.core.persisted.storage.Storage;
...
import julianh06.wynnextras.wtshim.models.combat.type.DamageDealtEvent;
import julianh06.wynnextras.wtshim.models.containers.containers.RaidRewardChestContainer;
import julianh06.wynnextras.wtshim.models.containers.event.ValuableFoundEvent;
...
import julianh06.wynnextras.wtshim.models.items.items.game.EmeraldItem;
import julianh06.wynnextras.wtshim.models.items.items.game.TomeItem;
import julianh06.wynnextras.wtshim.models.raid.bossbar.ParasiteOvertakenBar;
...
import julianh06.wynnextras.wtshim.models.raid.event.RaidNewBestTimeEvent;
import julianh06.wynnextras.wtshim.models.raid.event.RaidStartedEvent;
import julianh06.wynnextras.wtshim.models.raid.raids.NestOfTheGrootslangsRaid;
import julianh06.wynnextras.wtshim.models.raid.raids.OrphionsNexusOfLightRaid;
import julianh06.wynnextras.wtshim.models.raid.raids.RaidKind;
import julianh06.wynnextras.wtshim.models.raid.raids.TheCanyonColossusRaid;
import julianh06.wynnextras.wtshim.models.raid.raids.TheNamelessAnomalyRaid;
import julianh06.wynnextras.wtshim.models.raid.raids.TheWartornPalaceRaid;
import julianh06.wynnextras.wtshim.models.raid.scoreboard.RaidScoreboardPart;
import julianh06.wynnextras.wtshim.models.raid.type.HistoricRaidInfo;
...
import julianh06.wynnextras.wtshim.utils.MathUtils;
...
public final class RaidModel extends Model {
    ...
    public RaidModel() {
        // compat shim: shim Model has a no-arg constructor (no dependency list).
        super();
    ...
    public void onTitle(TitleSetTextEvent event) {
        Text component = event.getComponent(); // compat shim: Yarn Text
    ...
    private void interruptRaid() {
        ...
        // compat shim: Yarn Text + shim has no sendWynntilsPrefixMessage.
    ...
    public void onSlotClicked(ContainerClickEvent e) {
        ...
        if (Models.Container.getCurrentContainer() instanceof RaidRewardChestContainer raidRewardChest) {
            if (e.getSlotNum() == raidRewardChest.REROLL_REWARDS_SLOT) {
                // compat shim: LoreUtils returns List<Text> in Yarn; wrap into StyledText.
                StyledText rerollLoreConfirm =
                        StyledText.fromComponent(LoreUtils.getLore(e.getItemStack()).getFirst());
    ...
    public void onSetContent(ContainerSetContentEvent.Post event) {
        ...
        if (!rerollItem.isEmpty()) {
            // compat shim: LoreUtils returns List<Text> in Yarn; wrap into StyledText.
            StyledText rerollLoreConfirm = StyledText.fromComponent(LoreUtils.getLore(rerollItem).getFirst());
    ...
    private void processAspectItemFind(ItemStack itemStack, int slotId) {
        ...
        WynntilsMod.warn("[RaidModel] Unexpectedly found item \""
                + StyledText.fromComponent(itemStack.getName()).getStringWithoutFormatting() + "\" at slot "
