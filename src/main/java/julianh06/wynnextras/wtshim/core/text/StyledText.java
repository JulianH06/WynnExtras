 ...
 *  - ChatFormatting -> Formatting (PREFIX_CODE -> FORMATTING_CODE_PREFIX).
 *  - Component.empty()/literal() -> Text.empty()/Text.literal().
 *  - MutableComponent.create(contents) -> MutableText.of(content); Component#getContents -> getContent.
 *  - Style#applyTo(parent) -> Style#withParent(parent).
 * Dropped from the original (documented):
 *  - fromJson(JsonArray): unused in the shim, pulls in Gson + sprite-source subtypes via StyledTextPart.
 *  - StyledTextSerializer (Gson JsonSerializer/JsonDeserializer inner class): nothing in the shim
 *    serializes StyledText through it; the shim uses features.misc.StyledTextAdapter for Gson instead.
 */
...
import julianh06.wynnextras.wtshim.utils.type.IterationDecision;
...
public final class StyledText implements Iterable<StyledTextPart> {
    ...
    public static StyledText fromComponent(Text component) {
        ...
        while (!deque.isEmpty()) {
            ...
            // We use getContent here to get this and only this component's string.
            // Yarn: MutableComponent.create(getContents()) -> MutableText.of(getContent()).
            String componentString =
                    MutableText.of(current.getContent()).getString();
            ...
            // Yarn: Style#applyTo(parent) -> Style#withParent(parent).
            Style styleToFollowForChildren = current.getStyle().withParent(parentStyle);

            List<Pair<Text, Style>> siblingPairs = current.getSiblings().stream()
    ...
    public StyledText substring(int beginIndex, int endIndex, StyleType styleType) {
        ...
        for (StyledTextPart part : parts) {
            if (currentIndex >= beginIndex && currentIndex + part.length() < endIndex) {
                ...
                // Yarn: ChatFormatting.PREFIX_CODE -> Formatting.FORMATTING_CODE_PREFIX.
                if (beforeSubstring.endsWith(String.valueOf(Formatting.FORMATTING_CODE_PREFIX))
                        || includedSubstring.endsWith(String.valueOf(Formatting.FORMATTING_CODE_PREFIX))) {
