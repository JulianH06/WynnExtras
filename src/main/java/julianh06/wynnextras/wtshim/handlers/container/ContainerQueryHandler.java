...
/*
 *
 * Yarn adaptations (Mojmap -> Yarn):
 *   Component -> Text; AbstractContainerScreen -> HandledScreen; MenuType -> ScreenHandlerType;
 *   McUtils.containerMenu().containerId -> .syncId; ServerboundContainerClosePacket sends routed
 *   through ContainerUtils.closeContainer(int).
 * DEVIATION: the Wynntils LocalSoundEvent subscription (which silences the menu-click sound while a
 *   query runs) is dropped — no LocalSoundEvent exists in the shim. Purely cosmetic; queries still work.
 */
package julianh06.wynnextras.wtshim.handlers.container;
...
public final class ContainerQueryHandler extends Handler {
    ...
    private ScreenHandlerType<?> currentMenuType;
    ...
    public void runQuery(ContainerQueryStep firstStep) {
        ...
        if (McUtils.containerMenu() != null && McUtils.containerMenu().syncId != 0) {
    ...
    public void endAllQueries() {
        ...
        if (containerId != NO_CONTAINER) {
            ContainerUtils.closeContainer(containerId);
