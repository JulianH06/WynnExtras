...
import julianh06.wynnextras.features.shoppinglist.compat.ShoppingListWynnMarketSearchCompat;
...
public final class ShoppingListTradeMarketSearchService {
    ...
    private static final ShoppingListTradeMarketWorkflow WORKFLOW = new ShoppingListTradeMarketWorkflow();
    ...
    private static ShoppingListWynnMarketSearchCompat.SuppressionHandle wynnMarketSearchSuppression = null;

    private ShoppingListTradeMarketSearchService() {}
    ...
    public static void register() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(ShoppingListTradeMarketSearchService::tick);
    }
    ...
    public static Result searchOrCopy(String query) {
        ...
        ShoppingListScreenContext context = currentScreenContext(client);
        if (context == ShoppingListScreenContext.TRADE_MARKET_FILTER
                || context == ShoppingListScreenContext.TRADE_MARKET_DETAIL
        ...
        ShoppingListCursorRestoreService.recordBeforeShoppingListTradeMarketSearch();
        WORKFLOW.beginSearch(normalizedQuery, now);
        publish(Status.OPENING_TRADE_MARKET, normalizedQuery);

        if (context == ShoppingListScreenContext.TRADE_MARKET) {
    ...
    public static boolean isSearchPrompt(String message) {
        return ShoppingListTextCleaner.clean(message).contains(SEARCH_PROMPT);
    }
    ...
    private static void tick(MinecraftClient client) {
        ...
        if (WORKFLOW.state() == ShoppingListTradeMarketWorkflowState.OPENING_TRADE_MARKET
                && currentScreenContext(client) == ShoppingListScreenContext.TRADE_MARKET) {
        ...
        ShoppingListCursorRestoreService.TickResult cursorRestoreResult = ShoppingListCursorRestoreService.tick(client);
        if ((cursorRestoreResult == ShoppingListCursorRestoreService.TickResult.RESTORED
                || cursorRestoreResult == ShoppingListCursorRestoreService.TickResult.TIMED_OUT)
                && WORKFLOW.state() == ShoppingListTradeMarketWorkflowState.WAITING_FOR_RESULTS) {
    ...
    private static boolean isValidSearchSlot(MinecraftClient client, ScreenHandler handler, int slotIndex) {
        ...
        return stack != null
                && !stack.isEmpty()
                && ShoppingListTradeMarketSlotMatcher.isSearchFilterLabel(stack.getName().getString());
    }

    private static ShoppingListScreenContext currentScreenContext(MinecraftClient client) {
    ...
    private static void handleChatMessage(String message) {
        ...
        if (client != null && client.player != null && client.player.networkHandler != null) {
            ...
            boolean cursorRestoreArmed = ShoppingListCursorRestoreService.armForSearchResults();
    ...
    private static Result publish(Status status, String query) {
        if (status == Status.FAILED || status == Status.TIMED_OUT) {
            ShoppingListCursorRestoreService.clearPending();
            releaseWynnMarketSearchSuppression();
            lastStartedQuery = "";
            lastStartedAtMs = 0L;
        }
    ...
    private static void suppressWynnMarketSearch() {
        releaseWynnMarketSearchSuppression();
        wynnMarketSearchSuppression = ShoppingListWynnMarketSearchCompat.suppressDuringShoppingListSearch();
    }
