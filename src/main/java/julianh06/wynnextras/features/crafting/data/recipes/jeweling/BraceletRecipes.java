...
public class BraceletRecipes implements IRecipeData {
    ...
    public List<Pair<IMaterial, Integer>> getMaterials(int level) {
        if(level < 10) {
            ...
            return List.of(new Pair<>(Gem.GOLD, 4), new Pair<>(Oil.SALMON, 2));
        } else if (level < 40) {
            return List.of(new Pair<>(Gem.SANDSTONE, 6), new Pair<>(Oil.CARP, 3));
        } else if (level < 50) {
            return List.of(new Pair<>(Gem.IRON, 6), new Pair<>(Oil.ICEFISH, 3));
        } else if (level < 60) {
            return List.of(new Pair<>(Gem.SILVER, 8), new Pair<>(Oil.PIRANHA, 4));
        } else if (level < 70) {
            return List.of(new Pair<>(Gem.COBALT, 8), new Pair<>(Oil.KOI, 4));
        } else if (level < 80) {
            return List.of(new Pair<>(Gem.KANDERSTONE, 10), new Pair<>(Oil.GYLIA, 5));
        } else if (level < 90) {
