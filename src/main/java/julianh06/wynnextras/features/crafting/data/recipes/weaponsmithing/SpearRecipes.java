...
public class SpearRecipes implements IRecipeData {
    ...
    public List<Pair<IMaterial, Integer>> getMaterials(int level) {
        if(level < 10) {
            return List.of(new Pair<>(Wood.OAK, 2), new Pair<>(Ingot.COPPER, 1));
        } else if (level < 20) {
            return List.of(new Pair<>(Wood.BIRCH, 4), new Pair<>(Ingot.GRANITE, 2));
        } else if (level < 30) {
            return List.of(new Pair<>(Wood.WILLOW, 4), new Pair<>(Ingot.GOLD, 2));
        } else if (level < 40) {
            return List.of(new Pair<>(Wood.ACACIA, 6), new Pair<>(Ingot.SANDSTONE, 3));
        } else if (level < 50) {
            return List.of(new Pair<>(Wood.SPRUCE, 6), new Pair<>(Ingot.IRON, 3));
        } else if (level < 60) {
            return List.of(new Pair<>(Wood.JUNGLE, 8), new Pair<>(Ingot.SILVER, 4));
        } else if (level < 70) {
            return List.of(new Pair<>(Wood.DARK, 8), new Pair<>(Ingot.COBALT, 4));
        } else if (level < 80) {
            return List.of(new Pair<>(Wood.LIGHT, 10), new Pair<>(Ingot.KANDERSTONE, 5));
        } else if (level < 90) {
