...
public class AspectScreenSimple extends WEScreen {
    ...
    private int currentPage = 0; // 0 = Loot Pools, 1 = Gambits, 2 = My Aspects
    private static final int MAX_PAGE = 2; // Page 0: Loot Pools, Page 1: Gambits, Page 2: My Aspects
    ...
    private int[] calculateMaxRarityCounts(String[] raids) {
        ...
        for (String raid : raids) {
            List<LootPoolData.AspectEntry> aspects = LootPoolData.INSTANCE.getLootPool(raid);
    ...
    private void drawRaidColumn(DrawContext context, int x, int y, String raidCode, String raidName, int colWidth, int mouseX, int mouseY, int mythicSeparatorY, int fabledSeparatorY, int raidIndex) {
        ...
        // Get aspects
        List<LootPoolData.AspectEntry> aspects = LootPoolData.INSTANCE.getLootPool(raidCode);
        ...
        if (aspects.isEmpty()) {
            ...
            checkAspectHover(mouseX, mouseY, x, contentStartY - scrollOffset, aspects, "Mythic", "", colWidth);
            ...
            int fabledY = mythicSeparatorY + 20 - scrollOffset;
            checkAspectHover(mouseX, mouseY, x, fabledY, aspects, "Fabled", "", colWidth);
            ...
            int legendaryY = fabledSeparatorY + 20 - scrollOffset;
            checkAspectHover(mouseX, mouseY, x, legendaryY, aspects, "Legendary", "", colWidth);
    ...
    private void checkAspectHover(int mouseX, int mouseY, int x, int y, List<LootPoolData.AspectEntry> aspects, String rarity, String prefix, int colWidth) {
        ...
        int currentY = y + 12; // Match text position
        ...
        for (LootPoolData.AspectEntry aspect : filtered) {
            ...
            if (mouseX >= x + 20 && mouseX <= x + colWidth - 20 &&
                mouseY >= currentY - 5 && mouseY <= currentY + lineHeight - 5) {
    ...
    private void renderGambitsPage(DrawContext context, int mouseX, int mouseY) {
        ...
        if (!data.hasToday() || data.gambits.isEmpty()) {
            // No gambit data
            drawCenteredText(context, "§7No gambit data", centerX, startY + 150);
            ...
            for (int i = 0; i < Math.min(data.gambits.size(), 4); i++) {
                GambitData.GambitEntry gambit = data.gambits.get(i);
    ...
    private void renderMyAspectsPage(DrawContext context, int mouseX, int mouseY) {
        ...
        switch (activeStatus) {
            ...
            case FORBIDDEN:
                ...
                if (searchedPlayer.isEmpty()) {
                    drawCenteredText(context, "§7Click here to scan your aspects", centerX, logicalH / 2 + 30);
                }
                return;
            case UNAUTHORIZED:
    ...
    public boolean mouseClicked(Click click, boolean doubleClick) {
        ...
        if (button == 0 && shiftDown) {
            ...
            // Check if clicking on my aspect (page 2)
            if (currentPage == 2 && hoveredMyAspect != null) {
        ...
        // Check button clicks (on my aspects page - page 2)
        if (currentPage == 2) {
            ...
            if (!showOverview) {
                int maxButtonWidth = 160;
                int maxButtonHeight = 36;
                int maxSpacing = 12;
    ...
    public static void openGambits() {
        ...
        screen.currentPage = 1; // Go to gambits page
